...
xmeth2   = extrapol2()
xmethk   = extrapol3()
expl_o1 = time_rk2(mesh1, xmeth1)
expl_o2 = time_rk2(mesh1, xmeth2)
expl_o3  = time_rk2(mesh1, xmethk)
...
resultsk = expl_k.solve(field0, cfl, tsave)
...
for i in range(1,len(results1)):
    ...
    plot(mesh1.centers(), resultsk[i].qdata[0], style[i])
    labels.append("O1, t=%.1f"%results1[i].time)
    labels.append("O2, t=%.1f"%results2[i].time)
    labels.append("O3, t=%.1f"%results2[i].time)
