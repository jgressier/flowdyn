...
class euler(base.model):
    ...
    def prim2cons(self, pdata):  # qdata[ieq][cell] :
        ...
        rhoe = pdata[2] / (self.gamma - 1.0) + 0.5 * pdata[0] * V2
    ...
    def kinetic_energy(self, qdata):
        ...
        return 0.5 * qdata[1] ** 2 / qdata[0] if qdata[1].ndim == 1 else 0.5 * _vecsqrmag(qdata[1]) / qdata[0]
    ...
    @_vardict.register()
    def mach(self, qdata):
        return qdata[1] / np.sqrt(
            self.gamma * ((self.gamma - 1.0) * (qdata[0] * qdata[2] - 0.5 * qdata[1] ** 2))
        )

    @_vardict.register()
    def entropy(self, qdata):  # S/r
        return np.log(self.pressure(qdata) / qdata[0] ** self.gamma) / (self.gamma - 1.0)
    ...
    def ptot(self, qdata):
        gm1 = self.gamma - 1.0
        return self.pressure(qdata) * (1.0 + 0.5 * gm1 * self.mach(qdata) ** 2) ** (self.gamma / gm1)

    @_vardict.register()
    def rttot(self, qdata):
        ec = self.kinetic_energy(qdata)
        return ((qdata[2] - ec) * self.gamma + ec) / qdata[0] / self.gamma * (self.gamma - 1.0)
    ...
    def _Roe_average(self, rhoL, uL, HL, rhoR, uR, HR):
        ...
        uRoe = tmp * (uL + uR * Rrho)
        hRoe = tmp * (HL + HR * Rrho)
        cRoe = np.sqrt((hRoe - 0.5 * uRoe**2) * (self.gamma - 1.0))
    ...
    def numflux_centeredflux(self, pdataL, pdataR, direction=None):  # centered flux ; pL[ieq][face]
        ...
        # final flux
        Frho = 0.5 * (rhoL * unL + rhoR * unR)
        Frhou = 0.5 * ((rhoL * unL**2 + pL) + (rhoR * unR**2 + pR))
        FrhoE = 0.5 * ((rhoL * unL * HL) + (rhoR * unR * HR))
    ...
    def numflux_centeredmassflow(self, pdataL, pdataR, direction=None):  # centered flux ; pL[ieq][face]
        ...
        # final flux
        Frho = 0.5 * (rhoL * unL + rhoR * unR)
        Frhou = 0.5 * (Frho * (unL + unR) + pL + pR)
        FrhoE = 0.5 * Frho * (HL + HR)
    ...
    def numflux_hlle(self, pdataL, pdataR, direction=None):  # HLLE Riemann solver ; pL[ieq][face]
        ...
        sL = np.minimum(0.0, np.minimum(uRoe - cRoe, unL - np.sqrt(cL2)))
        sR = np.maximum(0.0, np.maximum(uRoe + cRoe, unR + np.sqrt(cR2)))

        # final flux
        Frho = (sR * rhoL * unL - sL * rhoR * unR + sL * sR * (rhoR - rhoL)) / (sR - sL)
        Frhou = (
            sR * (rhoL * unL**2 + pL) - sL * (rhoR * unR**2 + pR) + sL * sR * (rhoR * unR - rhoL * unL)
        ) / (sR - sL)
        FrhoE = (sR * (rhoL * unL * HL) - sL * (rhoR * unR * HR) + sL * sR * (rhoR * eR - rhoL * eL)) / (
            sR - sL
        )
    ...
    def numflux_hllc(self, pdataL, pdataR, direction=None):  # HLLC Riemann solver ; pL[ieq][face]
        ...
        sM = (pL - pR - rhoL * unL * (sL - unL) + rhoR * unR * (sR - unR)) / (
        ...
        SmoSSm = np.where(sM >= 0.0, sM / (sL - sM), sM / (sR - sM))
        SmUoSSm = np.where(sM >= 0.0, (sL - unL) / (sL - sM), (sR - unR) / (sR - sM))

        Frho = np.where(
            sM >= 0.0,
            np.where(sL >= 0.0, rhoL * unL, rhoL * sM * SmUoSSm),
            np.where(sR <= 0.0, rhoR * unR, rhoR * sM * SmUoSSm),
        )

        Frhou = np.where(
            sM >= 0.0,
            np.where(sL >= 0.0, Frho * uL + pL, Frho * uL + (pStar - pL) * SmoSSm + pStar),
            np.where(sR <= 0.0, Frho * uR + pR, Frho * uR + (pStar - pR) * SmoSSm + pStar),
        )

        FrhoE = np.where(
            sM >= 0.0,
            np.where(sL >= 0.0, rhoL * HL * unL, Frho * eL + (pStar * sM - pL * unL) * SmoSSm + pStar * sM),
            np.where(sR <= 0.0, rhoR * HR * unR, Frho * eR + (pStar * sM - pR * unR) * SmoSSm + pStar * sM),
        )
    ...
    def numflux_stegerwarming(self, pdataL, pdataR, direction=None):
        ...
        Fplus[1, :] = (
            ...
            / (2 * gam)
            * (
                (uL - np.sqrt(cL2)) * lbda_plus[0, :]
                + 2 * (gam - 1) * uL * lbda_plus[1, :]
                + (uL + np.sqrt(cL2)) * lbda_plus[2, :]
            )
        )
        Fplus[2, :] = (
            rhoL
            / (2 * gam)
            * (
                (HL - uL * np.sqrt(cL2)) * lbda_plus[0, :]
                + (gam - 1) * uL**2 * lbda_plus[1, :]
                + (HL + uL * np.sqrt(cL2)) * lbda_plus[2, :]
            )
        )
        ...
        Fminus[1] = (
            ...
            / (2 * gam)
            * (
                (uR - np.sqrt(cR2)) * lbda_minus[0]
                + 2 * (gam - 1) * uR * lbda_minus[1]
                + (uR + np.sqrt(cR2)) * lbda_minus[2]
            )
        )
        Fminus[2] = (
            rhoR
            / (2 * gam)
            * (
                (HR - uR * np.sqrt(cR2)) * lbda_minus[0]
                + (gam - 1) * uR**2 * lbda_minus[1]
                + (HR + uR * np.sqrt(cR2)) * lbda_minus[2]
            )
        )
    ...
    def numflux_vanleer(self, pdataL, pdataR, direction=None):
        ...
        subplus = np.array(
            [
                0.25 * rhoL * cL * (1.0 + machL) ** 2,
                0.25 * rhoL * cL * (1.0 + machL) ** 2 * 2 * cL / gamma * (G7 * machL + 1.0),
                0.25
                * rhoL
                * cL
                * (1.0 + machL) ** 2
        ...
        subminus = np.array(
            [
                -0.25 * rhoR * cR * (1.0 - machR) ** 2,
                -0.25 * rhoR * cR * (1.0 - machR) ** 2 * 2 * cR / gamma * (G7 * machR - 1.0),
                -0.25
                * rhoR
                * cR
                * (1.0 - machR) ** 2
        ...
        Fplus = np.where(machL >= 1.0, physicalL, np.where(machL <= -1.0, 0.0, subplus))
        Fminus = np.where(machR <= -1.0, physicalR, np.where(machR >= 1.0, 0.0, subminus))
    ...
    def numflux_ausm(self, pdataL, pdataR, direction=None):
        ...
        presL_plus = np.where(abs(machL) <= 1.0, 0.5 * pL * (1.0 + machL), np.where(machL > 0.0, pL, 0.0))

        machR_minus = np.where(abs(machR) <= 1.0, -0.25 * (machR - 1.0) ** 2, 0.5 * (machR - abs(machR)))
        presR_minus = np.where(abs(machR) <= 1.0, 0.5 * pR * (1.0 - machR), np.where(machR < 0.0, pR, 0.0))
        ...
        Frhou = np.where(
            mach_interface >= 0.0,
            mach_interface * rhoL * cL * uL + pres_interface,
            mach_interface * rhoR * cR * uR + pres_interface,
        )
        FrhoE = np.where(
            mach_interface >= 0.0, mach_interface * rhoL * cL * HE_L, mach_interface * rhoR * cR * HE_R
        )
    ...
    def timestep(self, data, dx, condition):
        ...
        dt = (
            condition
            * dx
            / (Vmag + np.sqrt(self.gamma * (self.gamma - 1.0) * (data[2] / data[0] - 0.5 * Vmag**2)))
        )
        return dt
...
class euler1d(euler):
    ...
    def _derived_fromprim(self, pdata, direction):
        ...
        H = c2 / (self.gamma - 1.0) + 0.5 * pdata[1] ** 2
    ...
    def bc_insub(self, direction, data, param):
        ...
        gmu = g - 1.0
        p = data[2]
        m2 = np.maximum(0.0, ((param['ptot'] / p) ** (gmu / g) - 1.0) * 2.0 / gmu)
        rh = param['ptot'] / param['rttot'] / (1.0 + 0.5 * gmu * m2) ** (1.0 / gmu)
    ...
    def bc_insub_cbc(self, direction, data, param):
        ...
        adiscri = g * (g + 1) / gmu * param['rttot'] - 0.5 * gmu * invcm**2
        a1 = (direction * invcm + np.sqrt(adiscri)) * gmu / (g + 1.0)
        u1 = invcm - direction * 2 * a1 / gmu
        f_m1sqr = 1.0 + 0.5 * gmu * (u1 / a1) ** 2
        rh1 = param['ptot'] / param['rttot'] / f_m1sqr ** (1.0 / gmu)
    ...
    def bc_outsub_qtot(self, direction, data, param):
        ...
        p = param['p']
        m2 = np.maximum(0.0, ((ptot / p) ** (gmu / g) - 1.0) * 2.0 / gmu)
        rho = ptot / rttot / (1.0 + 0.5 * gmu * m2) ** (1.0 / gmu)
    ...
    def bc_outsub_rh(self, direction, data, param):
        ...
        Ms2 = 1.0 + (pratio - 1.0) * (g + 1.0) / (2.0 * g)
        rhoratio = ((g + 1.0) * Ms2) / (2.0 + gmu * Ms2)
...
class euler2d(euler):
    ...
    def _derived_fromprim(self, pdata, direction):
        ...
        H = c2 / (self.gamma - 1.0) + 0.5 * _vecsqrmag(pdata[1])
    ...
    def _Roe_average_2d(self, rhoL, unL, UL, HL, rhoR, unR, UR, HR):
        ...
        URoe = tmp * (UL + UR * Rrho)
        hRoe = tmp * (HL + HR * Rrho)
        cRoe = np.sqrt((hRoe - 0.5 * _vecsqrmag(URoe)) * (self.gamma - 1.0))
    ...
    def numflux_centeredflux(self, pdataL, pdataR, direction=None):  # centered flux ; pL[ieq][face]
        ...
        # final flux
        Frho = 0.5 * (rhoL * unL + rhoR * unR)
        Frhou = 0.5 * ((rhoL * unL) * VL + pL * direction + (rhoR * unR) * VR + pR * direction)
        FrhoE = 0.5 * ((rhoL * unL * HL) + (rhoR * unR * HR))
        return [Frho, Frhou, FrhoE]
    ...
    def numflux_hlle(self, pdataL, pdataR, direction=None):  # HLLE Riemann solver ; pL[ieq][face]
        ...
        Frho = (sR * rhoL * unL - sL * rhoR * unR + sL * sR * (rhoR - rhoL)) / (sR - sL)
        Frhou = (
            sR * ((rhoL * unL) * VL + pL * direction)
            - sL * ((rhoR * unR) * VR + pR * direction)
            + sL * sR * (rhoR * VR - rhoL * VL)
        ) / (sR - sL)
        FrhoE = (sR * (rhoL * unL * HL) - sL * (rhoR * unR * HR) + sL * sR * (rhoR * etR - rhoL * etL)) / (
            sR - sL
        )
        return [Frho, Frhou, FrhoE]
