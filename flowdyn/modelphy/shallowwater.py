...
class shallowwater1d(base.model):
    ...
    def numflux_centeredflux(
        self, pdataL, pdataR, direction=None
        ...
        # final flux
        Fh = 0.5 * (hL * uL + hR * uR)
        Fq = 0.5 * ((hL * uL**2 + 0.5 * g * hL**2) + (hR * uR**2 + 0.5 * g * hR**2))
    ...
    def numflux_rusanov(self, pdataL, pdataR, direction=None):  # Rusanov flux ; pL[ieq][face]
        ...
        qL = hL * uL
        qR = hR * uR
        Fh = 0.5 * (qL + qR) - 0.5 * cmax * (hR - hL)
        Fq = 0.5 * ((qL * uL + 0.5 * g * hL**2) + (qR * uR**2 + 0.5 * g * hR**2)) - 0.5 * cmax * (qR - qL)
    ...
    def numflux_hll(self, pdataL, pdataR, direction=None):  # HLL flux ; pL[ieq][face]
        ...
        cL = np.sqrt(g * hL)
        cR = np.sqrt(g * hR)
        sL = np.minimum(0.0, np.minimum(uL - cL, uR - cR))
        sR = np.maximum(0.0, np.maximum(uL + cL, uR + cR))
        kL = sR / (sR - sL)
        kR = -sL / (sR - sL)
        qL = hL * uL
        qR = hR * uR
        Fh = kL * qL + kR * qR - kR * sR * (hR - hL)
        Fu = kL * (qL * uL + 0.5 * g * hL**2) + kR * (qR * uR + 0.5 * g * hR**2) - kR * sR * (qR - qL)
        return [Fh, Fu]
