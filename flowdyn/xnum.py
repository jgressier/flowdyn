...
class extrapol2(virtualmeth):
    ...
    def interp_face(self, mesh, data, grad):
        ...
        for i in range(len(data)):
            Ldata.append(np.zeros(nc + 1))
            Rdata.append(np.zeros(nc + 1))
            Ldata[i][1:] = data[i][:] + grad[i][0:-1] * (
                mesh.xf[1:] - mesh.xc[:]
            )  # data are cell based index, grad are facebased index
...
class extrapolk(virtualmeth):
    ...
    def interp_face(self, mesh, data, grad):
        ...
        for i in range(len(data)):
            ...
            Ldata[i][1:] = data[i][:] + (
                (1 - self.kprec) * grad[i][0:-1] + (1 + self.kprec) * grad[i][1:]
            ) / 2 * (mesh.xf[1:] - mesh.xc[:])
            Rdata[i][0:-1] = data[i][:] + (
                (1 - self.kprec) * grad[i][1:] + (1 + self.kprec) * grad[i][0:-1]
            ) / 2 * (mesh.xf[0:-1] - mesh.xc[:])
        return Ldata, Rdata
    ...
    def interp_face(self, mesh, data, field, neq, xgrad='none', ygrad='none'):
        ...
        for p in range(neq):
            if data[p].ndim == 2:
                ...
                for j in range(ny):
                    Ldata[p][:, fshift + (j + 1) * nx : fshift + (j + 2) * nx] = data[p][
                        :, j * nx : (j + 1) * nx
                    ]
        ...
        for p in range(neq):
            if data[p].ndim == 2:
                for j in range(ny):  # loop of cell rows
                    Ldata[p][:, j * (nx + 1) + 1 : (j + 1) * (nx + 1)] = (
                        data[p][:, j * nx : (j + 1) * nx]
                        + km * xgrad[p][:, (nx + 1) * j : j * (nx + 1) + nx]
                        + kp * xgrad[p][:, j * (nx + 1) + 1 : (j + 1) * (nx + 1)]
                    )
                    Rdata[p][:, j * (nx + 1) : (j + 1) * (nx + 1) - 1] = (
                        data[p][:, j * nx : (j + 1) * nx]
                        - km * xgrad[p][:, j * (nx + 1) + 1 : (j + 1) * (nx + 1)]
                        - kp * xgrad[p][:, (nx + 1) * j : j * (nx + 1) + nx]
                    )

                for j in range(ny):  # loop of cell rows
                    Ldata[p][:, fshift + (j + 1) * nx : fshift + (j + 2) * nx] = (
                        data[p][:, j * nx : (j + 1) * nx]
                        + km * ygrad[p][:, j * nx : (j + 1) * nx]
                        + kp * ygrad[p][:, nx * (j + 1) : (j + 2) * nx]
                    )
                    Rdata[p][:, fshift + j * nx : fshift + (j + 1) * nx] = (
                        data[p][:, j * nx : (j + 1) * nx]
                        - km * ygrad[p][:, nx * (j + 1) : (j + 2) * nx]
                        - kp * ygrad[p][:, j * nx : (j + 1) * nx]
                    )
...
class quick(extrapolk):
    ...
    def __init__(self):
        extrapolk.__init__(self, k=1.0 / 2.0)
...
class extrapol3(extrapolk):
    ...
    def __init__(self):
        extrapolk.__init__(self, k=1.0 / 3.0)
...
def minmod(a, b):
    p = a * b
    return np.where(p <= 0.0, 0, np.where(a > 0, np.minimum(a, b), np.maximum(a, b)))


def vanalbada(a, b):
    p = a * b
    return np.where(p <= 1e-40, 0.0, p * (a + b) / (a**2 + b**2 + 1e-20))


def vanleer(a, b):
    p = a * b
    s = np.abs(a + b) + 1.0e-20
    return np.where(p <= 1e-40, 0.0, 2 * p / s * np.sign(a))
...
def superbee(a, b):
    ...
    return np.where(
        ...
        np.where(
            a > 0,
            np.minimum(2 * np.minimum(a, b), np.maximum(a, b)),
...
class muscl(virtualmeth):
    ...
    def interp_face(self, mesh, data, grad):
        ...
        for i in range(len(data)):
            ...
            Rdata[i][0:-1] = data[i][:] + self.limiter(grad[i][0:-1], grad[i][1:]) * (
                mesh.xf[0:-1] - mesh.xc[:]
            )
        return Ldata, Rdata
